...
<div class="page">
    ...
    <div class="right-panel">
        ...
        <div class="login-card">
            ...
            <p class="form-eyebrow">Secret Admin Portal</p>
            <h2 class="form-title">Create <span>Admin.</span></h2>

            @if(session('error'))
                <div class="server-error"><i class='bx bx-error-circle'></i>{{ session('error') }}</div>
            @else
                <p class="form-desc">Fill in your details to create an admin account.</p>
            @endif
            ...
            <form method="POST" action="{{ route('admin.create.store') }}" onsubmit="return validateAdminForm(event)" novalidate>
                @csrf
                <div class="form-fields">
                    <div>
                        <div class="field-lbl">Admin/Staff ID</div>
                        <div class="input-wrap">
                            <i class='bx bx-id-card icon'></i>
                            <input type="text" name="student_id" placeholder="e.g., ADMIN-123" value="{{ old('student_id') }}" required autocomplete="off">
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="input-wrap">
                            ...
                            <input type="email" name="email" placeholder="admin@usep.edu.ph" value="{{ old('email') }}" required autocomplete="email">
                        </div>
                    </div>
                    <div>
                        <div class="field-lbl">Account Role</div>
                        <div class="input-wrap">
                            <i class='bx bx-shield icon'></i>
                            <select name="role" required>
                                <option value="" {{ !old('role') ? 'selected' : '' }} disabled>Select Role</option>
                                <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>Admin</option>
                                @if(isset($superAdminCount) && $superAdminCount < 3)
            ...
            <p class="signup-row"><a href="{{ route('login') }}">Back to Sign In</a></p>
        </div>
    </div>
</div>
...
<div class="loading-modal" id="loadingModal">
    <div class="loading-box">
        <div class="loader-ring-wrap">
            ...
            <div class="loader-icon"><i class='bx bx-shield-quarter'></i></div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.validateAdminForm = function (event) {
        ...
        if (!studentId) msgs.push('Admin/Staff ID is required.');
        ...
        if (!role) msgs.push('Account Role is required.');
    ...
    window.onload = function() {
        ...
        if (swstatus === 'success') {
            Swal.fire({
                ...
                text: swmessage || 'Admin account created successfully!',
